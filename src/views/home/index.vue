  ...
  methods: {
    ...
    getCalendarDays() {
      ...
      if (~~startDateArr[1] === ~~endDateArr[1]) {
        ...
        for (let day = 1; day < endDateArr[2]; day++) {
          // 结束月
          console.log('for2');
          this.date.list.push({
            year: endDateArr[0],
        ...
        if (this.eventInfo.isMouseDown) {
          ...
          for (let row = this.eventInfo.top; row <= this.eventInfo.bottom; row++) {
            for (let column = this.eventInfo.left; column <= this.eventInfo.right; column++) {
              ...
              if (row <= this.selectRangeTemp.bottom && row >= this.selectRangeTemp.top && column <= this.selectRangeTemp.right && column >= this.selectRangeTemp.left) {
    ...
    /**
       ...
       */
    outTable() {
      this.eventInfo.mouseover.row = this.eventInfo.mouseover.column = this.eventInfo.top = this.eventInfo.bottom = this.eventInfo.left = this.eventInfo.right = 0;
    }
  }
};
</script>
